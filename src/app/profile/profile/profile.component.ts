...
export class ProfileComponent implements OnInit {
  ...
  public set profile(value: Profile) {
    value.skills = value.skills.sort((s1, s2) => compareBy(s1, s2, 'percentage', true))
    value.educations = value.educations.sort((s1, s2) => compareBy(s1, s2, 'startDate', true))
    value.experiences = value.experiences.sort((s1, s2) => compareBy(s1, s2, 'startDate', true))
    this._profile = value;
  }
  ...
  ngOnInit(): void {
    ...
    this.navbarItems = [
      { icon: 'pi pi-info-circle', label: 'About', command: ()=> this.scrollToCard(this.about)},
      { icon: 'pi pi-briefcase', label: 'Experience', command: ()=> this.scrollToCard(this.experience)},
      { icon: 'pi pi-bolt', label: 'Skills', command: ()=> this.scrollToCard(this.skills)},
      { icon: 'pi pi-book', label: 'Education', command: ()=> this.scrollToCard(this.education)},
      { icon: 'pi pi-phone', label: 'Contact Me', command: ()=> this.scrollToCard(this.contact)},
    ]
  }
